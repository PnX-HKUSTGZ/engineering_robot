...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        ...
        int action_monitor_rate = 100;
        std::string chasis_joint_name = "";
  ...
  public:
    ...
    : ParamListener(parameters_interface, rclcpp::get_logger("mecanum_chasis_drive_controller"), prefix) {
    ...
    bool try_update_params(Params & params_in) const {
      ...
      if (lock.owns_lock()) {
        const bool is_old = params_in.__stamp != params_.__stamp;
        if (is_old) {
          params_in = params_;
          return true;
        }
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        ...
        if (param.get_name() == (prefix_ + "action_monitor_rate")) {
          auto validation_result = not_empty<int>(param);  
          if(      !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.action_monitor_rate = param.as_int();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        // chasis_joint_name
        if (param.get_name() == (prefix_ + "chasis_joint_name")) {
          auto validation_result = not_empty<std::string>(param);  
          if(      !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.chasis_joint_name = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        ...
        if (param.get_name() == (prefix_ + "kinematics.wheels_radius")) {
          auto validation_result = gt<double>(param, 0.0);  
          if(      !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
    ...
    void declare_params(){
      ...
      param = parameters_interface_->get_parameter(prefix_ + "chasis_joint_name");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      auto validation_result = not_empty<std::string>(param);
      if(!validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'chasis_joint_name': {}", validation_result.error()));
      }
      updated_params.chasis_joint_name = param.as_string();
